...
# Check logs on VPS
ssh <VPS_USER>@<VPS_IP> "docker logs --tail=50 luanti-voxelibre-server 2>&1 | grep -i error"
```
...
```bash
ssh <VPS_USER>@<VPS_IP> "cd $PROJECT_PATH && cp -r server/worlds server/worlds_EMERGENCY_BACKUP_$(date +%Y%m%d_%H%M%S)"
