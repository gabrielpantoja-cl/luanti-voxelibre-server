...
# Check logs on VPS
ssh gabriel@<VPS_IP> "docker logs --tail=50 luanti-voxelibre-server 2>&1 | grep -i error"
```
...
```bash
ssh gabriel@<VPS_IP> "cd /home/gabriel/luanti-voxelibre-server && cp -r server/worlds server/worlds_EMERGENCY_BACKUP_$(date +%Y%m%d_%H%M%S)"
