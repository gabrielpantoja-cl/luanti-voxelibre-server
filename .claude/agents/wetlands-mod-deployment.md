...
- **VPS**: cloud provider con SSH access (<VPS_USER>@<VPS_IP>)
...
rsync -avz --progress server/mods/$MOD_NAME/ <VPS_USER>@<VPS_IP>:<PROJECT_PATH>/server/mods/$MOD_NAME/
...
check_server_health() {
    ...
    local disk_usage=$(df <PROJECT_PATH> | tail -1 | awk '{print $5}' | sed 's/%//')
...
WORLD_DIR="<PROJECT_PATH>/server/worlds"
MODS_DIR="<PROJECT_PATH>/server/mods"
CONFIG_DIR="<PROJECT_PATH>/server/config"
MAX_BACKUPS=10
...
BACKUP_DIR="<PROJECT_PATH>/server/backups"
TARGET_DIR="<PROJECT_PATH>/server"
...
- **Orchestration**: Reportar al `orchestrator` sobre estado de deployments
...
5. **Orchestration**: Reporta al `orchestrator` sobre estado general del servidor
...
ssh <VPS_USER>@<VPS_IP> "cd <PROJECT_PATH> && docker-compose exec -T luanti-server cat /config/.minetest/worlds/world/world.mt"

# 2. Listar solo mods habilitados
ssh <VPS_USER>@<VPS_IP> "cd <PROJECT_PATH> && docker-compose exec -T luanti-server grep 'load_mod.*= true' /config/.minetest/worlds/world/world.mt"

# 3. Listar solo mods deshabilitados
ssh <VPS_USER>@<VPS_IP> "cd <PROJECT_PATH> && docker-compose exec -T luanti-server grep 'load_mod.*= false' /config/.minetest/worlds/world/world.mt"

# 4. Verificar estado de un mod específico
ssh <VPS_USER>@<VPS_IP> "cd <PROJECT_PATH> && docker-compose exec -T luanti-server grep '^load_mod_animal_sanctuary' /config/.minetest/worlds/world/world.mt"

# 5. Buscar duplicados (líneas que aparecen más de una vez)
ssh <VPS_USER>@<VPS_IP> "cd <PROJECT_PATH> && docker-compose exec -T luanti-server cat /config/.minetest/worlds/world/world.mt | sort | uniq -d"
```
...
if ssh <VPS_USER>@<VPS_IP> "test -d <PROJECT_PATH>/server/mods/$MOD_NAME"; then
...
MOD_CONFIG=$(ssh <VPS_USER>@<VPS_IP> "cd <PROJECT_PATH> && docker-compose exec -T luanti-server grep '^load_mod_$MOD_NAME' /config/.minetest/worlds/world/world.mt")
...
DUPLICATE_COUNT=$(ssh <VPS_USER>@<VPS_IP> "cd <PROJECT_PATH> && docker-compose exec -T luanti-server grep '^load_mod_$MOD_NAME' /config/.minetest/worlds/world/world.mt | wc -l")

if [ "$DUPLICATE_COUNT" -gt 1 ]; then
    echo "❌ ERROR: Mod configurado $DUPLICATE_COUNT veces (duplicado)"
    echo "📋 Líneas duplicadas:"
    ssh <VPS_USER>@<VPS_IP> "cd <PROJECT_PATH> && docker-compose exec -T luanti-server grep '^load_mod_$MOD_NAME' /config/.minetest/worlds/world/world.mt"
    exit 1
else
...
MOD_LOAD_LOG=$(ssh <VPS_USER>@<VPS_IP> "cd <PROJECT_PATH> && docker-compose logs --tail=100 luanti-server | grep -i '$MOD_NAME'")
...
if [ $? -eq 0 ]; then
    ...
    ssh <VPS_USER>@<VPS_IP> "cd <PROJECT_PATH> && docker-compose exec -T luanti-server sed -i 's/^load_mod_$MOD_NAME = .*/load_mod_$MOD_NAME = true/' /config/.minetest/worlds/world/world.mt"
else
    # Mod nuevo, agregar
    ssh <VPS_USER>@<VPS_IP> "cd <PROJECT_PATH> && docker-compose exec -T luanti-server sh -c 'echo \"load_mod_$MOD_NAME = true\" >> /config/.minetest/worlds/world/world.mt'"
fi
...
sleep 15
ssh <VPS_USER>@<VPS_IP> "cd <PROJECT_PATH> && docker-compose ps | grep luanti-server"
```
