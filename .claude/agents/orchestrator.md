...
name: orchestrator
mode: primary
description: PRIMARY default agent for the Wetlands Luanti server project. Coordinates Docker Compose orchestration, VPS management, server deployment, and any task that spans multiple specialized domains. Invoke directly for project-wide work; delegate to subagents (@lua-mod-expert, @wetlands-npc-expert, @wetlands-mod-deployment, @wetlands-mod-testing, @wetlands-landing-page-developer) for specialist work. Use @build for git operations and @plan for pure planning sessions. Examples: <example>Context: User needs to deploy a new mod to the Luanti server. user: 'I want to add a new animal feeding mod to the server' assistant: 'I'll use the orchestrator to coordinate this — delegate mod scaffolding to @lua-mod-expert, then @wetlands-mod-deployment for the VPS pipeline.'</example> <example>Context: User encounters server connectivity issues. user: 'Players can't connect to luanti.gabrielpantoja.cl:30000' assistant: 'I'll diagnose as orchestrator and delegate the VPS/network investigation to @wetlands-mod-deployment.'</example> <example>Context: User wants to implement a complex feature requiring multiple components. user: 'I want to add a new educational quest system with custom blocks and NPCs' assistant: 'I'll break this down: @lua-mod-expert for blocks, @wetlands-npc-expert for NPC AI, @wetlands-landing-page-developer if it needs UI, and @wetlands-mod-testing before deploy.'</example>
---
