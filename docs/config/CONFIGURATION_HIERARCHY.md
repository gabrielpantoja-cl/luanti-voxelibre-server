...
ssh gabriel@<VPS_IP> "cd /home/gabriel/Vegan-Wetlands && docker-compose logs luanti-server | grep creative"
```
...
ssh gabriel@<VPS_IP> "cd /home/gabriel/Vegan-Wetlands && cat server/config/luanti.conf | grep -E '(creative|damage|default_privs)'"

# Ver qué mods están cargados
ssh gabriel@<VPS_IP> "cd /home/gabriel/Vegan-Wetlands && cat server/worlds/world/world.mt | grep load_mod"

# Ver logs del servidor para errores de configuración
ssh gabriel@<VPS_IP> "cd /home/gabriel/Vegan-Wetlands && docker-compose logs --tail=50 luanti-server | grep -i error"
```
