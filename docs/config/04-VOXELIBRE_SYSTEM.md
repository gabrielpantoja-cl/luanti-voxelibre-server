...
```bash
# En el VPS
ssh gabriel@<VPS_IP> "cd /home/gabriel/luanti-voxelibre-server && docker compose logs luanti-server | grep -i 'error\|warn\|conflict'"
```
...
```bash
ssh gabriel@<VPS_IP> "cd /home/gabriel/luanti-voxelibre-server && docker compose ps && ss -tulpn | grep :30000"
```
...
```bash
# 1. Backup del mundo
ssh gabriel@<VPS_IP> "cd /home/gabriel/luanti-voxelibre-server && ./scripts/backup.sh"
