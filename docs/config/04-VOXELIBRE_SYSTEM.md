...
```bash
# En el VPS
ssh <VPS_USER>@<VPS_IP> "cd $PROJECT_PATH && docker compose logs luanti-server | grep -i 'error\|warn\|conflict'"
```
...
```bash
ssh <VPS_USER>@<VPS_IP> "cd $PROJECT_PATH && docker compose ps && ss -tulpn | grep :30000"
```
