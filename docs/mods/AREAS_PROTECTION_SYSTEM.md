...
```bash
ssh gabriel@<VPS_IP> 'cd /home/gabriel/luanti-voxelibre-server && docker compose exec -T luanti-server sqlite3 /config/.minetest/worlds/world/auth.sqlite "SELECT a.name, GROUP_CONCAT(up.privilege, \", \") FROM auth a LEFT JOIN user_privileges up ON a.id = up.id WHERE a.name IN (\"gabo\", \"pepelomo\", \"gaelsin\", \"loxos\") GROUP BY a.name;"'
```
