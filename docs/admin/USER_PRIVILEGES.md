...
**Capacidades Admin**:
- `ban`, `kick`, `mute`, `unban` - Moderación
- `server`, `shutdown` - Control del servidor
- `privs`, `password` - Gestión de usuarios
- `rollback_check` - Anti-griefing
- `settime`, `time` - Control del mundo

#### 🟢 Creative Standard (13 privilegios)
...
- `interact`, `shout` - Interacción básica
...
VoxeLibre incluye protección de construcciones por defecto:

1. **`mcl_spawn` protection**: Área de spawn protegida automáticamente
2. **Player ownership**: Los bloques colocados por un jugador están vinculados a ese jugador
3. **Permission system**: Solo el propietario puede modificar/destruir sus bloques

### Privilegios Críticos para Anti-Griefing
...
- **`rollback_check`**: Tienen **5 admins**
  - Permite ver quién modificó bloques
...
- No hay distinción entre "moderador" y "admin completo"
...
# Ver quién tiene un privilegio específico
...
- `bring`: Traer jugadores a tu ubicación
...
### Privilegios de Protección
- `protect`: Proteger áreas
...
- `worldedit`: Usar WorldEdit (edición masiva de bloques)
...
- **Fecha de asignación de admins**: 2025-11-15 (pepelomo, gaelsin, loxos, gapi)
- **Servidor en producción**: No se realizó reinicio durante la investigación
