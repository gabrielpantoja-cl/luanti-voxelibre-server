...
scp gabriel@<VPS_IP>:/home/gabriel/luanti-voxelibre-server/docker-compose.yml /tmp/docker-compose-vps.yml
scp gabriel@<VPS_IP>:/home/gabriel/luanti-voxelibre-server/scripts/rotate-backups-container.sh /tmp/rotate-backups-vps.sh
```
...
```bash
# Download both files
scp gabriel@<VPS_IP>:/home/gabriel/luanti-voxelibre-server/docker-compose.yml /tmp/
scp gabriel@<VPS_IP>:/home/gabriel/luanti-voxelibre-server/scripts/rotate-backups-container.sh /tmp/
...
scp gabriel@<VPS_IP>:/home/gabriel/luanti-voxelibre-server/server/config/luanti.conf /tmp/
...
ssh gabriel@<VPS_IP> 'cd /home/gabriel/luanti-voxelibre-server && git status --porcelain'
...
```bash
ssh gabriel@<VPS_IP> 'cd /home/gabriel/luanti-voxelibre-server && git status'
```
...
ssh gabriel@<VPS_IP> 'cd /home/gabriel/luanti-voxelibre-server && git diff'

# View file-by-file changes
ssh gabriel@<VPS_IP> 'cd /home/gabriel/luanti-voxelibre-server && git status -v'
```
