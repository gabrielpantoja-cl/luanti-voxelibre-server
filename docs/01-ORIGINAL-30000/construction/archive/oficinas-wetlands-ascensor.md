...
```bash
ssh <VPS_USER>@<VPS_IP> "cd $PROJECT_PATH && docker-compose logs luanti-server 2>&1 | grep -E '(celevator|machine|drive|controller)' | tail -30"
```
...
ssh <VPS_USER>@<VPS_IP> "cd $PROJECT_PATH && docker-compose logs -f luanti-server"

# Buscar acciones específicas del ascensor
ssh <VPS_USER>@<VPS_IP> "cd $PROJECT_PATH && docker-compose logs luanti-server 2>&1 | grep celevator | tail -50"
