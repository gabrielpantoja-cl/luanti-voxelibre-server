...
ssh <VPS_USER>@<VPS_IP> "cd $PROJECT_PATH && git fetch origin main && git reset --hard origin/main"
```
...
```
VPS: $PROJECT_PATH/server/landing-page
...
2. Confirmar que el repo en VPS esta actualizado: `ssh <VPS_USER>@<VPS_IP> "cd $PROJECT_PATH && git log -1 --oneline"`
