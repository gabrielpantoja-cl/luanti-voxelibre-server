...
if [ -z "$IP_TO_UNBLOCK" ] || [ -z "$REASON" ]; then
    ...
    echo "📋 Ejemplo: $0 <BROMA_IP> \"Usuario legítimo bloqueado por error\""
    exit 1
fi
...
if ! ssh -o ConnectTimeout=5 gabriel@<VPS_IP> "echo 'SSH OK'" >/dev/null 2>&1; then
...
# Backup luanti.conf
ssh gabriel@<VPS_IP> "cd /home/gabriel/Vegan-Wetlands && cp server/config/luanti.conf server/config/luanti.conf.backup.unblock_$TIMESTAMP"

# Backup ipban.txt (si existe)
ssh gabriel@<VPS_IP> "cd /home/gabriel/Vegan-Wetlands && docker-compose exec -T luanti-server cp /config/.minetest/worlds/world/ipban.txt /config/.minetest/worlds/world/ipban.txt.backup.$TIMESTAMP 2>/dev/null || echo 'ipban.txt no existe'"
...
LUANTI_CONF_BLOCK=$(ssh gabriel@<VPS_IP> "cd /home/gabriel/Vegan-Wetlands && grep 'deny_access.$IP_TO_UNBLOCK' server/config/luanti.conf || echo 'NO_FOUND'")
IPBAN_BLOCK=$(ssh gabriel@<VPS_IP> "cd /home/gabriel/Vegan-Wetlands && docker-compose exec -T luanti-server grep '$IP_TO_UNBLOCK' /config/.minetest/worlds/world/ipban.txt 2>/dev/null || echo 'NO_FOUND'")
...
# Verificar remoción
VERIFY_LUANTI=$(ssh gabriel@<VPS_IP> "cd /home/gabriel/Vegan-Wetlands && grep '$IP_TO_UNBLOCK' server/config/luanti.conf || echo 'REMOVED'")
...
ssh gabriel@<VPS_IP> "cd /home/gabriel/Vegan-Wetlands && docker-compose exec -T luanti-server sh -c \"grep -v '$IP_TO_UNBLOCK' /config/.minetest/worlds/world/ipban.txt > /tmp/ipban_temp 2>/dev/null && mv /tmp/ipban_temp /config/.minetest/worlds/world/ipban.txt 2>/dev/null || echo 'ipban.txt procesado'\""

# Verificar remoción
VERIFY_IPBAN=$(ssh gabriel@<VPS_IP> "cd /home/gabriel/Vegan-Wetlands && docker-compose exec -T luanti-server grep '$IP_TO_UNBLOCK' /config/.minetest/worlds/world/ipban.txt 2>/dev/null || echo 'REMOVED'")
...
SERVER_STATUS=$(ssh gabriel@<VPS_IP> "cd /home/gabriel/Vegan-Wetlands && docker-compose ps luanti-server --format 'table {{.Status}}'" | tail -1)
...
ssh gabriel@<VPS_IP> "cd /home/gabriel/Vegan-Wetlands && [ ! -f $UNBLOCK_LOG ] && echo '# Log de IPs Desbloqueadas' > $UNBLOCK_LOG"

# Añadir entrada al log
ssh gabriel@<VPS_IP> "cd /home/gabriel/Vegan-Wetlands && echo '## $(date)' >> $UNBLOCK_LOG"
ssh gabriel@<VPS_IP> "cd /home/gabriel/Vegan-Wetlands && echo '- **IP**: $IP_TO_UNBLOCK' >> $UNBLOCK_LOG"
ssh gabriel@<VPS_IP> "cd /home/gabriel/Vegan-Wetlands && echo '- **Razón**: $REASON' >> $UNBLOCK_LOG"
ssh gabriel@<VPS_IP> "cd /home/gabriel/Vegan-Wetlands && echo '- **Método**: deny_access + ipban.txt' >> $UNBLOCK_LOG"
ssh gabriel@<VPS_IP> "cd /home/gabriel/Vegan-Wetlands && echo '- **Backup timestamp**: $TIMESTAMP' >> $UNBLOCK_LOG"
ssh gabriel@<VPS_IP> "cd /home/gabriel/Vegan-Wetlands && echo '' >> $UNBLOCK_LOG"
