...
# Backup luanti.conf
ssh gabriel@<VPS_IP> "cd /home/gabriel/luanti-voxelibre-server && cp server/config/luanti.conf server/config/luanti.conf.backup.unblock_$TIMESTAMP"

# Backup ipban.txt (si existe)
ssh gabriel@<VPS_IP> "cd /home/gabriel/luanti-voxelibre-server && docker-compose exec -T luanti-server sh -c 'if [ -f /config/.minetest/worlds/world/ipban.txt ]; then cp /config/.minetest/worlds/world/ipban.txt /config/.minetest/worlds/world/ipban.txt.backup.$TIMESTAMP; echo \"ipban.txt backed up\"; else echo \"ipban.txt no existe\"; fi'"
...
LUANTI_CONF_BLOCK=$(ssh gabriel@<VPS_IP> "cd /home/gabriel/luanti-voxelibre-server && grep 'deny_access.$IP_TO_UNBLOCK' server/config/luanti.conf || echo 'NO_FOUND'")
IPBAN_BLOCK=$(ssh gabriel@<VPS_IP> "cd /home/gabriel/luanti-voxelibre-server && docker-compose exec -T luanti-server grep '$IP_TO_UNBLOCK' /config/.minetest/worlds/world/ipban.txt 2>/dev/null || echo 'NO_FOUND'")
...
ssh gabriel@<VPS_IP> "cd /home/gabriel/luanti-voxelibre-server && sed -i '/deny_access\.$IP_TO_UNBLOCK/d' server/config/luanti.conf"

# Verificar remoción
VERIFY_LUANTI=$(ssh gabriel@<VPS_IP> "cd /home/gabriel/luanti-voxelibre-server && grep '$IP_TO_UNBLOCK' server/config/luanti.conf || echo 'REMOVED'")
...
ssh gabriel@<VPS_IP> "cd /home/gabriel/luanti-voxelibre-server && docker-compose exec -T luanti-server sh -c \"grep -v '$IP_TO_UNBLOCK' /config/.minetest/worlds/world/ipban.txt > /tmp/ipban_temp 2>/dev/null && mv /tmp/ipban_temp /config/.minetest/worlds/world/ipban.txt 2>/dev/null || echo 'ipban.txt procesado'\""

# Verificar remoción
VERIFY_IPBAN=$(ssh gabriel@<VPS_IP> "cd /home/gabriel/luanti-voxelibre-server && docker-compose exec -T luanti-server grep '$IP_TO_UNBLOCK' /config/.minetest/worlds/world/ipban.txt 2>/dev/null || echo 'REMOVED'")
...
SERVER_STATUS=$(ssh gabriel@<VPS_IP> "cd /home/gabriel/luanti-voxelibre-server && docker-compose ps luanti-server --format 'table {{.Status}}'" | tail -1)
...
ssh gabriel@<VPS_IP> "cd /home/gabriel/luanti-voxelibre-server && [ ! -f $UNBLOCK_LOG ] && echo '# Log de IPs Desbloqueadas' > $UNBLOCK_LOG"

# Añadir entrada al log
ssh gabriel@<VPS_IP> "cd /home/gabriel/luanti-voxelibre-server && cat >> $UNBLOCK_LOG << 'EOF'
## $(date '+%Y-%m-%d %H:%M:%S')
- **IP**: $IP_TO_UNBLOCK
- **Razón**: $REASON
