...
ssh gabriel@<VPS_IP> "cd /home/gabriel/Vegan-Wetlands && docker-compose exec -T luanti-server sh -c 'if [ -f /config/.minetest/worlds/world/ipban.txt ]; then cp /config/.minetest/worlds/world/ipban.txt /config/.minetest/worlds/world/ipban.txt.backup.$TIMESTAMP; echo \"ipban.txt backed up\"; else echo \"ipban.txt no existe\"; fi'"
...
ssh gabriel@<VPS_IP> "cd /home/gabriel/Vegan-Wetlands && cat >> $UNBLOCK_LOG << 'EOF'
## $(date '+%Y-%m-%d %H:%M:%S')
- **IP**: $IP_TO_UNBLOCK
- **Razón**: $REASON
