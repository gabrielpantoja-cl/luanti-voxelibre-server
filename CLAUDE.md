...
ssh gabriel@<VPS_IP> "docker logs --since='2m' luanti-voxelibre-server 2>&1 | grep -i 'error\|warning\|my_mod'"

# Enable a mod on VPS (world.mt must be edited in BOTH host and container)
ssh gabriel@<VPS_IP> "echo 'load_mod_my_mod = true' >> /home/gabriel/luanti-voxelibre-server/server/worlds/world/world.mt"
ssh gabriel@<VPS_IP> "docker exec luanti-voxelibre-server sh -c 'echo \"load_mod_my_mod = true\" >> /config/.minetest/worlds/world/world.mt'"
```
